...
getCubes row rowNumber =
  zip [(rowNumber, x, 0,0) | x <- [0..(length row)]] row

getMapCone:: ((Int,Int, Int, Int), (Int,Int, Int, Int)) -> CubeMap
getMapCone  ((xl,yl,zl,wl),(xh, yh,zh, wh)) =
  Map.fromList $ zip [(x,y,z,w) | x <- [xl-1..xh+1], y <- [yl-1..yh+1], z <- [zl-1..zh+1],w <- [wl-1..wh+1]] (repeat '.')

getAdjacentCubes:: Coords -> CubeMap -> [Char]
getAdjacentCubes cube cubeMap =
  let pos = [(x,y,z,w) | x <- [-1..1], y <- [-1..1], z <- [-1..1], w <- [-1..1]] \\ [(0,0,0,0)]
